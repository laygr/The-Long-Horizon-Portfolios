...
def main(regressions, filtered_by_metadata):
    ...
    filtered_regressions = filtered_regressions[filtered_regressions['M'] >= 11]
    ...
    eliminated_regressions = eliminated_regressions[eliminated_regressions['M'] >= 11]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--regressions',          help='path to the file with the regressions data')
    parser.add_argument('--filtered_by_metadata', help='path to the file with tickers filtered by metadata')
    parser.add_argument('--filtered_output',      help='path to the file with tickers filtered by metadata')
    parser.add_argument('--eliminated_output',      help='path to the file with tickers filtered by metadata')
    ...
    filtered_regressions, eliminated_regressions = main(regressions, filtered_by_metadata)
